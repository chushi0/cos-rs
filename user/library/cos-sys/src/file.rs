...
pub fn create(path: &[u8]) -> Result<()> {
    ...
    let error = unsafe { syscall!(idx::IDX_FILE_CREATE, path_ptr, path_len) };
...
pub fn open(path: &[u8]) -> Result<u64> {
    ...
    let error = unsafe { syscall!(idx::IDX_FILE_OPEN, path_ptr, path_len, handle_ptr) };
...
pub fn get_pos(handle: u64) -> Result<u64> {
    ...
    let error = unsafe { syscall!(idx::IDX_FILE_GET_POS, handle, pos_ptr) };
    SyscallError::to_result(error).map(|_| unsafe { pos.assume_init() })
}

pub fn set_pos(handle: u64, pos: u64) -> Result<()> {
    let error = unsafe { syscall!(idx::IDX_FILE_SET_POS, handle, pos) };
    SyscallError::to_result(error)
}

pub fn close(handle: u64) -> Result<()> {
    let error = unsafe { syscall!(idx::IDX_FILE_CLOSE, handle) };
    SyscallError::to_result(error)
}
