...
use crate::{
    ...
    idx::{IDX_DEBUG, IDX_SUB_DEBUG_GET_CHAR, IDX_SUB_DEBUG_INFO, IDX_SUB_DEBUG_PUT_CHAR},
    syscall,
};

pub fn info() {
    unsafe {
        syscall!(IDX_DEBUG, IDX_SUB_DEBUG_INFO);
    }
}
...
pub fn get_char() -> Result<u8> {
    ...
    let error = unsafe { syscall!(IDX_DEBUG, IDX_SUB_DEBUG_GET_CHAR, char_ptr) };
...
pub fn put_char(char: u8) -> Result<()> {
    ...
    let error = unsafe { syscall!(IDX_DEBUG, IDX_SUB_DEBUG_PUT_CHAR, char_ptr) };
